...
function renderHypothesis() {
  ...
  const resultAspectModel = {
    ...
    practice: [
      ...
      ["Активных дней", "2 → 4 дня из 7", "Считаем дни, когда ученик сделал хотя бы одно задание; базовый уровень — 2 дня в неделю.", 2, 2, { max: 7, baseline: "База: 2 дня из 7", target: "Цель: 4 дня из 7" }],
  ...
  const workPlanModel = {
    ...
    practice: {
      ...
      totalDays: 5,
      work: [
        ["Генератор 6-денного плану", "2 дні", "Розкласти задачі за днями, складністю і темами уроку."],
        ["Стани учня", "1 день", "Почато, виконано, пропущено, батарейка витрачена, день уроку."],
        ["Серія і батарейки", "1.5 дні", "Захистити серію після пропуску без зняття відповідальності."],
        ["Перевірка вчителем", "0.5 дня", "Approve / Edit для задач перед публікацією плану."]
      ]
    },
    review: {
      title: "Повторення перед уроком",
      totalDays: 2,
      work: [
        ["Генератор питань повторення", "1 день", "3–5 питань з пройденого матеріалу, адаптованих до учня."],
        ["Результати для вчителя", "1 день", "Вчитель бачить відповіді до початку уроку і може скоригувати план."]
      ]
    },
    preview: {
      title: "Прев'ю нового матеріалу",
      totalDays: 2,
      work: [
        ["Коротке прев'ю теми", "1 день", "2–3 нові концепції наступного уроку у форматі картки або короткого відео."],
        ["Стан знайомості для вчителя", "1 день", "Вчитель бачить, скільки учнів переглянули прев'ю, до початку уроку."]
      ]
    },
    game: {
      title: "Геймифікація і прогрес",
      totalDays: 5,
      work: [
        ["Бали і серія", "1 день", "Нарахування балів за виконані задачі з урахуванням складності."],
        ["Батарейки захисту серії", "1 день", "Захист серії після пропуску без повного скидання прогресу."],
        ["Бейджі і досягнення", "1.5 дні", "Безпечні бейджі для учня і сім'ї без тиску."],
        ["Запобіжники якості", "1.5 дні", "Не давати бонуси, якщо падає точність або зростає залежність від підказок."]
      ]
    }
  };
  ...
  const hypothesisTabs = normalizedHypotheses.map((item, i) => {
    ...
    return `
      <button
        class="hypothesis-tab${i === activeHypothesisIndex ? " is-active" : ""}"
        type="button"
        data-hyp-index="${i}"
  ...
  const hypothesisDetail = `
    <div class="hypothesis-detail-grid">
      ...
      <article class="hypothesis-detail-card">
